...
// Конфигурация бота для локального тестирования
const botConfig = {
...
// Флаги для отладки
const DEBUG_ACTIONS = false; // false - убрать вывод действий

// Функция создания и подключения бота
function createBot() {
  console.log(`Попытка подключения к локальному серверу ${botConfig.host}:${botConfig.port}...`);
  ...
  bot.on('login', () => {
    isConnected = true;
    console.log(`✅ Бот ${bot.username} подключен к локальному серверу`);
...
// Запуск бота
console.log('🚀 Запуск бота для локального тестирования');
console.log(`📡 Сервер: ${botConfig.host}:${botConfig.port}`);
